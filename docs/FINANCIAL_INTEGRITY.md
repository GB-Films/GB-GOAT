...
- Every new main budget item advances a revision on the project in the same commit. Area activation reads a fresh budget snapshot and checks that revision in its transaction. A concurrent item creation causes activation to stop instead of omitting the new row.
...
| Area activation | Fresh server read, transactional row and revision checks; activation stops if one has a payment or a new row appears |
